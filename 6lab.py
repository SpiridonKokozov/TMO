...
def preprocess_data(data_in):
    ...
    scale_cols = ['upperwhisker', 'Logged GDP per capita']
    new_cols = []
    sc1 = MinMaxScaler()
    ...
    X = data_out[new_cols]
    Y = data_out['lowerwhisker'].astype(int)
...
st.write(data['Regional indicator'].value_counts())
...
ax.scatter(x=data['Social support'], y=data['Healthy life expectancy'])
plt.xlabel("Social support")
plt.ylabel("Healthy life expectancy")
st.pyplot(fig)

f1, ax = plt.subplots()
sns.boxplot(x=data['Social support'])
st.pyplot(f1)

st.subheader('Масштабирование данных')
f, ax = plt.subplots()
plt.hist(data['Social support'], 50)
plt.show()
st.pyplot(f)
...
ax = plt.scatter(X_test['upperwhisker_scaled'], Y_test, marker='o', label='Тестовая выборка')
plt.scatter(X_test['upperwhisker_scaled'], Y_predict, marker='.', label='Предсказанные данные')
plt.legend(loc='lower right')
plt.xlabel('upperwhisker_scaled')
plt.ylabel('strength')
...
forest_3 = RandomForestRegressor(n_estimators=7, oob_score=True, random_state=5)
forest_3.fit(X, Y)
...
fig1 = plt.figure(figsize=(7, 5))
ax = plt.scatter(X_test['upperwhisker_scaled'], Y_test, marker='o', label='Тестовая выборка')
plt.scatter(X_test['upperwhisker_scaled'], Y_predict3, marker='.', label='Предсказанные данные')
plt.legend(loc='lower right')
plt.xlabel('upperwhisker_scaled')
plt.ylabel('strength')
plt.plot(n_estimators_1)
...
ax = plt.scatter(X_test['upperwhisker_scaled'], Y_test, marker='o', label='Тестовая выборка')
plt.scatter(X_test['upperwhisker_scaled'], Y_grad_pred, marker='.', label='Предсказанные данные')
plt.legend(loc='lower right')
plt.xlabel('upperwhisker_scaled')
plt.ylabel('strength')
...
grad1 = GradientBoostingRegressor(n_estimators=75, max_features=1, min_samples_leaf=0.04, random_state=10)
...
fig1 = plt.figure(figsize=(7, 5))
ax = plt.scatter(X_test['upperwhisker_scaled'], Y_test, marker='o', label='Тестовая выборка')
plt.scatter(X_test['upperwhisker_scaled'], Y_grad_pred1, marker='.', label='Предсказанные данные')
plt.legend(loc='lower right')
plt.xlabel('upperwhisker_scaled')
plt.ylabel('strength')
plt.plot(n_estimators_1)
...
plt.scatter(X_test['upperwhisker_scaled'], Y_test, marker='s', label='Тестовая выборка')
plt.scatter(X_test['upperwhisker_scaled'], lr_y_pred, marker='o', label='Предсказанные данные')
plt.legend(loc='lower right')
plt.xlabel('upperwhisker_scaled')
plt.ylabel('strength')
plt.show()
